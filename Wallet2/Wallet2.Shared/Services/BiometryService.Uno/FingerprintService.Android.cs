...
namespace BiometryService
{
	public class FingerprintService : IFingerprintService
	{
		...
		private void OnAuthenticationError(int code, string message)
		{
			switch (code)
			{
				case BiometricPrompt.InterfaceConsts.ErrorUserCanceled:
				case BiometricPrompt.InterfaceConsts.ErrorNegativeButton:
		...
		private BiometricPrompt.CryptoObject BuildAsymmetricCryptoObject(string keyName, string signatureName)
		{
			if (this.Log().IsEnabled(LogLevel.Debug))
			{
		...
		public async Task Enroll(CancellationToken ct)
		{
			...
			var result = _BiometricManager.CanAuthenticate();
			if (result == BiometricManager.BiometricSuccess || result == BiometricManager.BiometricErrorNoneEnrolled)
			{
		...
		public IObservable<bool> GetAndObserveIsEnabled() => _canAuthenticate.Select(x =>
			x == BiometricManager.BiometricSuccess
		);

		public IObservable<bool> GetAndObserveIsSupported() => _canAuthenticate.Select(x =>
			x == BiometricManager.BiometricErrorNoneEnrolled ||
			x == BiometricManager.BiometricSuccess
		);
